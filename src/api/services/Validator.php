...
class Validator
{
    ...
    public function isValidParams(array $params, string $context, array $error): array
    {
        switch ($context) {
            case self::CONTEXT_REGISTER:
                $errors = [];
                $errors = $this->isValidRegisterParams($params, $errors);
                return $errors;
                break;

            case self::CONTEXT_LOGIN:
                $errors = [];
                $errors = $this->isValidLoginParams($params, $error);
                return $errors;
                break;

            case self::CONTEXT_UPDATE_USER:
                $errors = [];
                $errors = $this->isValidUpdateUserParams($params, $error);
                return $errors;
                break;
        }
    }

    private function isValidRegisterParams(array $params, array $errors): array
    {
        $errors = [];
    ...
    private function isValidLoginParams(array $params, $errors): array
    {
        $errors = [];
    ...
    private function isValidUpdateUserParams(array $params, $errors): array
    {
        $errors = [];
        return $errors;
    }
